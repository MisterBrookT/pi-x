...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("bench", {
    description: "Pix checks: /bench speed or /bench doctor",
    getArgumentCompletions: (prefix) => ["speed", "doctor"]
    ...
    handler: async (rawArgs, ctx) => {
      ...
      if (action === "speed") {
        ...
        const pix: number[] = [];
        for (let i = 0; i < 3; i++) {
          naive.push(await timed(["--no-extensions", "--list-models", "openai-codex"]));
          pix.push(await timed(["--list-models", "openai-codex"]));
        }
        const average = (values: number[]) => Math.round(values.reduce((a, b) => a + b, 0) / values.length);
        ctx.ui.notify(`Pi ${average(naive)}ms avg (${naive.join(", ")})\nPix ${average(pix)}ms avg (${pix.join(", ")})\nOverhead ${average(pix) - average(naive)}ms`, "info");
        return;
      }
      if (action !== "doctor") {
        ctx.ui.notify("Usage: /bench speed or /bench doctor", "error");
        return;
      }
