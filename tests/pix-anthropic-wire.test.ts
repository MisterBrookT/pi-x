...
check("User-Agent is current Claude Code CLI", h["user-agent"] === "claude-cli/2.1.280 (external, cli)", h["user-agent"]);
...
console.log("\n=== HEADER ENFORCEMENT (model.headers must not clobber fingerprint) ===");
{
	...
	srv.close();
	check("model.headers cannot override User-Agent", hijacked["user-agent"] === "claude-cli/2.1.280 (external, cli)", hijacked["user-agent"]);
